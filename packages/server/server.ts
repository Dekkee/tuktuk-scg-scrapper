...
app.get('/api/list', async function(req, resp) {
    ...
    console.log(
        `list request: name: ${queryObject.search_query}, page: ${queryObject.page}`
    );

    const answer = await (
        await fetch(`https://starcitygames.com/search.php?${query}`)
    ).text();
...
app.get('/api/get', async function(req, resp) {
    ...
    const answer = await (
        await fetch(`https://www.starcitygames.com/${preparedName}`)
    ).text();
...
app.listen(port, function() {
    console.log(
        colors.cyan(`Tuktuk server is running at http://localhost:${port}`)
    );
});
